...
class pbiDatasetAPI:
  ...
  def getPBITableRows(self, dataFrame,epoch_id =None):
    ...
    for row in dataFrame.rdd.toLocalIterator():
      ...
      for col in dataFrame.columns:
        rowString[col]= row[col]
      dataRows.append(rowString)
    dataString["rows"]= dataRows
    result = str(json.dumps(dataString))
    return result
  def getPBITableRowsStream(self, dataFrame,epoch_id =None):
    """Create data rows in JSON-format from a DataFrame"""
    dataRows = []
    for row in dataFrame.rdd.toLocalIterator():
      rowString = {}
  ...
  def executePBIOperation(  self
    ...
    if datasetName is not None:
      ...
      allKeys = self.executePBIRequest(endpoint = operationURL, method = "get").content
      allKeys =  allKeys.decode().strip()
      allKeys = json.loads(allKeys)
      allKeys = dict(allKeys)["value"]
    ...
    # Push Datasets - Datasets PostRows
      ...
      for tableName, dataFrame in zip(tableNames, dataFrames):
        operationURL = "{groupPrefix}datasets/{datasetKey}/tables/{tableName}/rows".format(groupPrefix = groupPrefix, datasetKey = datasetKey, tableName = tableName)
        dataFrameRows = {} 
        if dataFrame.isStreaming ==False:
          # Create a request body in JSON-format based on the data in dataFrames
          print("before error")
          dataFrameRows["rows"] =  self.getPBITableRows(dataFrame)
        else:
          dataFrameRows["rows"] = [{}]
          print(dataFrameRows)
          dataFrame.writeStream.foreachBatch(dataFrameRows["rows"].append(self.getPBITableRows)).start()    

        dataFrameRowsString = str(json.loads(dataFrameRows))
        body = '''
                    {
                        "rows": [
                                    ''' + dataFrameRowsString + '''
